...
func TestSourceUnavailable(t *testing.T) {
	p := newProber(t, "")
	_, err := p.Probe(context.Background(), plugin.ProbeRequest{
		Source:  netip.MustParseAddr("192.0.2.99"),
...
func TestFamilyMismatch(t *testing.T) {
	p := newProber(t, "")
	_, err := p.Probe(context.Background(), plugin.ProbeRequest{
		Source: lo, Target: netip.MustParseAddr("::1"), Count: 1, Timeout: time.Second,
	})
...
func TestConfig(t *testing.T) {
	...
	for _, y := range []string{"", "port: 53", "packet_interval: 5ms"} {
		c, _ := plugin.ConfigFromYAML(y)
