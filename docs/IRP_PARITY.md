...
The `commit` scorer (#18) is off unless `scorer.type` is `commit`. Its performance score matches `weighted`. When telemetry has a fresh billable 95th (`usage_mbps`, or the outbound 95th when the mode is `separate`) and the flow source reports per-prefix volume, it moves prefixes off a provider that is over `commit_mbps` onto one with room for that volume. A move that would increase loss is refused unless `loss_override` is set. `cc_disable` leaves a provider out of these moves. `balance` `equal` or `proportional` balances providers that share `group`; `off` only relieves over-commit. `precedence` (lower is preferred, omitted means 100) orders destinations, and the highest precedence is used only when every lower one lacks room. Each improvement is tagged `performance` or `commit` and counts toward `max_improvements`. Performance moves are not overridden. The prefix must be in the learned RIB. Switching the scorer back to `weighted` withdraws commit improvements. Observe stays the default.
