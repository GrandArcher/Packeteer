...
The `cost` scorer (#19) is off unless `scorer.type` is `cost`, and needs `providers[].cost` (price per Mbps) on at least two non-excluded providers. Its performance score matches `weighted`. It moves a prefix to the cheapest priced provider whose path is inside `floor`: at most `max_loss_pct` (default 0) above the lowest loss and `max_rtt` (default 10ms) above the lowest RTT among usable, non-excluded providers. The destination must be cheaper than native. Decide re-checks the floor and the price for every cost move, withdraws an active cost steer at once when it leaves the floor (with a `hold_time` cooldown), and applies the RIB, allowlist, cap, and cooldown rules. `precedence: performance` (default) lets performance moves win; `precedence: cost` keeps a native path inside the floor and, when native is outside it, prefers the cheapest path inside the floor over the fastest one. Improvements are tagged `cost`. `cost_delta` and `est_savings` (price difference times prefix volume) are on `/api/improvements`, and `packeteer_estimated_savings` sums them; the full savings report is #23. The cost scorer does not do commit control. Switching back to `weighted` and restarting withdraws cost improvements. Observe stays the default. The FRR lab's cost job announces a cost-cause route, withdraws it when the cheap path leaves the floor, and checks SIGTERM and SIGKILL.
