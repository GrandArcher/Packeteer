...
2. **`Start(ctx)`.** Begins background work in goroutines and must not block. Plugins start in this order: sources, probers, scorer, policies, telemetry, notifiers, announcer. If one fails, the ones already started are stopped.
...
- `static`: `targets: [{prefix, host?, weight?, mbps?}]`. `host` must be inside `prefix` and defaults to the first address. `mbps` (0–100000000) is an optional declared rate, decimal megabits per second, for the `commit` scorer. Zero omits the prefix. This does not announce.
