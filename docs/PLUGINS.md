...
| `source` | `Targets(ctx) ([]Target, error)` | `static`, `flow`, `traceroute`, `vip` | `exec` |
...
- `udp`: `port` (default 33434), `packet_interval` (default 100ms). A UDP reply or an ICMP port-unreachable both count as a reply. The kernel reports the unreachable on the connected socket, so this prober does not need a raw socket. It is not in the default chain; add it after `tcp` when you want that fallback.
...
- `traceroute`: discovers the probe host for each configured prefix. `targets` is required (`prefix`, optional `host` inside the prefix, optional `weight`). `max_hops` (default 16, 1–64), `probes` per hop (default 3, 1–10), `min_replies` (default 2, or `probes` when that is smaller; a hop is stable only when one address answers at least this many times and there is no tie), `timeout` per probe (default 500ms, max 5s), `port` (default 33434), `source` (optional local address; empty uses the default route), `interval` (default 5m, 1s–24h; the trace is cached). When the configured host answers, it stays the probe host. Otherwise the highest stable TTL is used, which is the responsive hop closest to the destination. Three silent hops after a stable one end the trace. Loopback, link-local, multicast, and unspecified answers are ignored unless they are the destination. The prefix on the target does not change, and the discovered address is not announced. If `source` is set and that address cannot be bound, the source fails for the round and the other sources continue.
- `vip`: prefixes and ASNs probed on their own interval. `interval` is required (at least 1s). At least one of `prefixes` (`prefix`, optional `host` inside it) or `asns` (non-zero, no duplicates) is required. A default route is rejected. Configured prefixes are always returned. ASN entries expand to learned prefixes whose AS path contains a listed ASN, and only while the RIB view is ready. The same prefix from another source keeps that source's host; the shorter positive interval wins, so listing a prefix here still speeds it up when flow or static also has it. The global probe rate limit applies. Set `interval` below `probe.interval` and well under the staleness window, or the measurements go stale between probes and improvements withdraw. This source does not announce.
