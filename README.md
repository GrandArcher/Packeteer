...
`mode: inject` is opt-in. The `gobgp` announcer then advertises the exact learned prefix for each allowlisted improvement on that same iBGP session: the provider's next hop, your `local_pref`, `packeteer_community`, and `no-export`. It does not synthesize more-specifics. The edge must accept only that community from Packeteer and must not export it to eBGP. See [docs/mikrotik.md](docs/mikrotik.md) and [docs/routers.md](docs/routers.md). Stopping the container, or switching back to `observe`, withdraws the routes. So does a withdraw of a prefix the router was still advertising, and so does probe data older than about three intervals — including when a probe round never finishes. The router stopping sending the native path because Packeteer's route became best does not: that improvement stays until flip-back or `improvement_ttl`. Graceful restart is never enabled.
