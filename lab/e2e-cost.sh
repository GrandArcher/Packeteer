#!/bin/bash
# Cost-cause injection against the same FRR edge as lab/e2e.sh.
# transit-b is cheaper and the fixed prober keeps it inside the cost floor
# and the latency threshold, so the route is a cost steer. Documentation
# prefix and private ASN only.
set -euo pipefail

root=$(cd "$(dirname "$0")/.." && pwd)
cd "$root"
export PACKETEER_LAB_CONFIG="$root/lab/packeteer-cost.yaml"
...
cp lab/probes/cost-inside.yaml "$lab_dir/state.yaml"
...
echo "waiting for the cost steer"
wait_route present
logs=$("${compose[@]}" logs --no-color packeteer)
if ! printf '%s\n' "$logs" | grep -q 'cause=cost'; then
	echo "announced route was not a cost improvement" >&2
	dump_bgp
	exit 1
fi
if printf '%s\n' "$logs" | grep -q 'cause=performance'; then
	echo "cost lab produced a performance improvement" >&2
	dump_bgp
	exit 1
fi
...
wait_route absent
logs=$("${compose[@]}" logs --no-color packeteer)
if ! printf '%s\n' "$logs" | grep -q 'outside performance floor'; then
	echo "withdraw was not a floor exit" >&2
	dump_bgp
	exit 1
fi

echo "cheap path back inside the floor (after the cooldown)"
flip_probes lab/probes/cost-inside.yaml
wait_route present
