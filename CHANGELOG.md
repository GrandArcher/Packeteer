...
- `commit` scorer. Keeps each provider's billable 95th under its commit by moving prefixes that have flow volume onto a provider with room, and can balance a provider group (`equal` or `proportional`). `precedence` is the last resort. A move that increases loss is refused unless `loss_override` is set. Improvements are tagged `performance` or `commit`, share `max_improvements`, and still require the learned RIB. The default scorer remains `weighted`. Switching back withdraws commit improvements.
